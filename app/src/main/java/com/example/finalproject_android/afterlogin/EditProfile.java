...
import com.example.finalproject_android.models.FileUtils;
...
public class EditProfile extends AppCompatActivity {
    ...
    public void updateUserProfile() {
        ...
        if (profilePictureUri != null) {
            File file = new File(FileUtils.getPath(this, profilePictureUri));
            // Sử dụng MIME type chính xác (image/jpeg hoặc image/png)
            RequestBody fileBody = RequestBody.create(MediaType.parse("image/jpeg"), file); // Hoặc "image/png" tùy vào loại ảnh
            profilePicturePart = MultipartBody.Part.createFormData("image", file.getName(), fileBody);
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == 100) {
            ...
            avatar.setImageURI(profilePictureUri);
        }
    }
